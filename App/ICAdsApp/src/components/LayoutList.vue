<template>
  <div id="layoutList" class="">
    <header
      class="hidden md:grid w-full grid-cols-5 rounded border py-2 bg-white border-charcoal50 px-4 items-center h-[42px]"
    >
    ...
    <main class="w-full mt-4">
      ...
      >
        ...
        <div class="flex items-center justify-end pr-4 gap-4">
          <button class="btn-secondary hidden lg:block"  @click="$router.push({ name: 'export', query: { initialLayout: layout.id } })">Export</button>
          <button @click="onDeleteLayout(layout.id)" class="flex items-center">
...
const onDeleteLayout = (id) => {
  $vfm.show({
    ...
    slots: {
      title: 'Delete layout',
      description: 'Really delete this layout?',
    },
  });
};
